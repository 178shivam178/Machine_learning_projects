...
class TomBertForMMSequenceClassification(PreTrainedBertModel):
    ...
    def __init__(self, config, num_labels=2, pooling="first"):
        super(TomBertForMMSequenceClassification, self).__init__(config)
        self.num_labels = num_labels
        self.pooling = pooling
        self.bert = BertModel(config)
...
class TomBertNoPoolingForMMSequenceClassification(PreTrainedBertModel):
    ...
    def __init__(self, config, num_labels=2):
        super(TomBertNoPoolingForMMSequenceClassification, self).__init__(config)
        self.num_labels = num_labels
        self.bert = BertModel(config)
        self.s2_bert = BertModel(config)
        # self.s2_bert = self.bert
        ...
        self.text_pooler = BertText16Pooler(config)
    ...
    def forward(self, input_ids, s2_input_ids, visual_embeds_att, token_type_ids=None, s2_type_ids=None,
        ...
        s2_cross_encoder = self.ent2img_attention(s2_output, converted_vis_embed_map, extended_img_mask)
        s2_cross_output_layer = s2_cross_encoder[-1]
        # s2_cross_output = self.ent2img_pooler(s2_cross_output_layer) #original for comment
        #################s2_cross_output, _ = s2_cross_output_layer.max(1)

        # transpose_img_embed = s2_cross_output.unsqueeze(1) # original for comment
        # text_img_output = torch.cat((transpose_img_embed, sequence_output), dim=1) # original for comment
        text_img_output = torch.cat((s2_cross_output_layer, sequence_output), dim=1) # [H^v; H^s]
        #################text_img_output = torch.cat((transpose_img_embed, sequence_output[:,1:,:]), dim=1)
        ###########text_img_output = torch.cat((s2_cross_output_layer, sequence_output), dim=1)

        comb_attention_mask = added_attention_mask[:, 33:]  # only the first 16 dimension is for image # [H^v; H^s]
        # comb_attention_mask = added_attention_mask[:, 48:]  # only the first dimension is for image # original for comment
        ###########comb_attention_mask = torch.cat((s2_mask, added_attention_mask[:, 49:]), dim=-1)
        extended_attention_mask = comb_attention_mask.unsqueeze(1).unsqueeze(2)
        extended_attention_mask = extended_attention_mask.to(dtype=next(self.parameters()).dtype)  # fp16 compatibility
        extended_attention_mask = (1.0 - extended_attention_mask) * -10000.0
